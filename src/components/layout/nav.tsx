...
export function Nav() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full pt-3 sm:pt-4 px-3 sm:px-4 pointer-events-none">
      <div className="mx-auto max-w-[820px] relative pointer-events-auto">
        {/* Floating Pill Nav Bar */}
        <nav
          className={cn(
            "w-full border border-neutral-200/80 bg-white/95 px-4 sm:px-6 py-2 shadow-sm backdrop-blur-md transition-all duration-300",
            menuOpen ? "rounded-2xl shadow-lg border-neutral-300" : "rounded-full",
            scrolled && !menuOpen && "shadow-md bg-white/95 border-neutral-300/80",
          )}
        >
          ...
          {/* Mobile Menu Panel inside rounded-2xl when open */}
          {menuOpen ? (
              ...
              <div className="flex flex-col gap-1">
                {LINKS.map((link) => (
                  <Link
                    ...
                    to={link.to}
                    hash={link.hash}
                    className="rounded-lg px-3 py-2.5 font-sans text-sm tracking-wide text-fg hover:bg-neutral-100 transition-colors"
                ...
                <div className="pt-2">
                  <Button asChild className="w-full rounded-full text-xs uppercase tracking-wider h-10">
